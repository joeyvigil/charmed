...
const Messages = ({ user }) => {
  ...
  React.useEffect(() => {
    const fetchMessages = async () => {
      ...
      const response = await fetch(`https://charmed-backend.onrender.com/users/messages/${selectedMatch.id}/page/${page}`, {
        headers: {
 ...
 React.useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    const response = await fetch(`https://charmed-backend.onrender.com/messages`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
      },
      body: JSON.stringify({ content: sendText, sender_id: user.id, receiver_id: selectedMatch.id })
    });
    ...
    if (response.ok) {
      ...
      // Refresh messages
      const fetchMessages = async () => {
        const response = await fetch(`https://charmed-backend.onrender.com/users/messages/${selectedMatch.id}/page/1`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  return (
        ...
        <div className="container ">
                    ...
                    {matches.map((match) => (
                      <div key={match.id}>
                        <div className="match-details">
                          ...
                          <button className="btn btn-sm mt-2" onClick={() => setSelectedMatch(match)}>Chat</button>
                        </div>
                      </div>
                    ))}
                  </>
            ...
            <div className="flex-container">
              ...
              <p className="text-center">selected match: {selectedMatch ? `${selectedMatch.first_name} ${selectedMatch.last_name}` : 'None'}, page number: {page}</p>
              <div className="text-center">
              <button className="btn btn-primary mx-2" onClick={() => setPage(page - 1)}>Previous Page</button>
              <button className="btn btn-primary mx-2" onClick={() => setPage(page + 1)}>Next Page</button>
              </div>
                        ...
                        <div className="match-details">
                          ...
                          <div>
                            {message.sender_id === user.id ? (
                              ...
                              <span className="color-them">{selectedMatch.first_name}:</span>
                            )}
                            &nbsp;
                            {message.content}
                          </div>
